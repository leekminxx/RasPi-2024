...
trigPin = 20
echoPin = 16
sensor_pin = 18  # DHT11 센서 데이터 핀 설정 (예시로 D18 핀 사용)
...
class WindowClass(QMainWindow, form_class):
  def __init__(self):
    ...
    self.btnStartDht.setVisible(False)
    self.btnStopDht.setVisible(False)
    ...
    try:
      ...
      self.measure_dht_thread = threading.Thread(target=self.measure_dht)
      self.measure_dht_thread.start()
    except RuntimeError as error:
      print(f"Error initializing DHT11 sensor: {error}")
      self.dhtDevice = None
  ...
  def btnDistanceFunction(self):
    ...
    t1 = threading.Thread(target=self.measure_distance_thread)
    t1.start()
    ...
    while True:
      if self.dhtDevice:
        try:
          ...
          print(f'Temp : {temperature_c}C / Humid : {humidity}%')
          self.update_lcd_dht(temperature_c, humidity)
        except RuntimeError as error:
  ...
  def update_lcd_dht(self, temperature, humidity):
