...
class WindowClass(QMainWindow, form_class):
    ...
    def turnOnRed(self):
        ...
        self.updateLabel("Red", "빨간색")

    def turnOnGreen(self):
        GPIO.output(red, GPIO.HIGH)
        GPIO.output(green, GPIO.LOW)
        GPIO.output(blue, GPIO.HIGH)
        self.updateLabel("Green", "초록색")

    def turnOnBlue(self):
        GPIO.output(red, GPIO.HIGH)
        GPIO.output(green, GPIO.HIGH)
        GPIO.output(blue, GPIO.LOW)
        self.updateLabel("Blue", "파란색")
    ...
    def updateLabel(self, color_code, text):
        color_dict = {"Red": "#FF0000", "Green": "#00FF00", "Blue": "#0000FF"}
        self.label_led.setText(f"<font color='{color_dict.get(color_code, 'black')}'>{text}</font>")
